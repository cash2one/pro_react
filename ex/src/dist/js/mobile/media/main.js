...
app = $.extend(app, {
	...
	initLoadTip: function initLoadTip() {
		...
		this.els.loadBlank = $('<div class="advices-analy-spread-detail-load tip" style="display: none;">' + '<div class="wrapper">' + '<div class="l">' + '<span class="step">提示</span>' + '</div>' + '<div class="r">' + '<section class="tit">暂无数据</section>' + '<section class="state">您添加的自动标签暂时没有相关文章。您可以添加新的自动标签或者耐心等待。</section>' + '</div>' + '</div>' + '</div>').appendTo('body');
	},
	...
	initLoadBar: function initLoadBar() {
		this.els.loadBar = $("<div class=\"load-bar\"><span class=\"inner\" style=\"width: 0\"></span></div>").appendTo('.advices-analy-media');
	},
	initToggleBar: function initToggleBar() {
		var toggles = $('#toggle-part .toggle-item'),
		    _this = this;
		toggles.each(function (i, el) {
			if ($(el).attr('data-time') == _this.state.timeRange) $(el).addClass('active');
		}).click(function () {
...
app = $.extend(app, {
	getData: function getData() {
		...
		if (this.configs.mediaType == 'company') {
			restArt.query.read('count', {
				...
				if (data.count == 0) {
					_this2.state.loadEnd = true;
					_this2.state.isBlank = true;
				} else {
					_this2.state.loadTxt = '统计数据加载完毕......';
					_this2.state.loadStep++;
					_this2.state.artTotal = data.count;
				}
				_this2.updateLoadTip();
				return data;
			}).then(function (data) {
				if (!data.result) return;
				$.when(_this2.getSrcData(), _this2.getMediaCount()).done(function () {
					_this2.state.srcMidsSelect = _this.state.srcData[0].category;
					var res = _this2.getMidsData();
					if (res) {
						res.done(function () {
							_this2.update();
						});
	...
	getSrcData: function getSrcData() {
		var type = this.configs.mediaType,
		    id,
		    timeRange = this.dealTimeRange();
		if (type == 'event') id = this.configs.eventId;else if (type == 'company') id = this.configs.companyId;
	...
	getMidsData: function getMidsData() {
		var _this = this,
		    mid = this.state.srcMidsSelect,
		    timeRange = this.dealTimeRange();
...
app = $.extend(app, {
	...
	updateChartZaiti: function updateChartZaiti() {
		var data = this.dealZaiti(this.state.srcData);
		this.chartZaiti.setOption(data.opts);
	},
	updateChartZhengfu: function updateChartZhengfu() {
		var data = this.dealZhengfu(this.state.srcData);
		this.chartZhengfu.setOption(data.opts);
	},
	updateChartMids: function updateChartMids() {
		var mid = this.state.srcMidsSelect;
		var data = this.dealMids(this.state.srcMidsData[mid]);
		this.chartMids.setOption(data);
	},
	updateChartMidsNavi: function updateChartMidsNavi() {
		var data = this.state.srcData,
		    select = this.state.srcMidsSelect,
		    items = '';
		$.each(data, function (i, dat) {
			items = items + '<li class="item' + (select == dat.category ? " active" : '') + '" data-category="' + dat.category + '">' + '<span class="txt">' + dat.category_name + '</span>' + '</li>';
		});
		$('#chart_mids_navi').html(items);
	},
	updateLoadTip: function updateLoadTip() {
		this.updateLoadBar();
		return;
		...
		if (!loadEnd) {
			if (!loadTip) {
				this.els.loadTip = loadTip = $('<div class="advices-analy-spread-detail-load">' + '<div class="wrapper">' + '<div class="l">' + '<span class="step"></span>' + '</div>' + '<div class="r">' + '<section class="tit">数据分析中</section>' + '<section class="state">{this.state.loadtxt}</section>' + '<section class="bar">' + '<span class="inner pct50 active"></span>' + '</section>' + '</div>' + '</div>' + '</div>');
			}
			...
			if (isBlank) {
				...
				loadBlank.show();
			} else $('.advices-analy-media').removeClass('loading');
		}
	},
...
app = $.extend(app, {
	dealZaiti: function dealZaiti(data) {
		var opts,
		    legends = [],
		    seriesData = [],
		    artTotal = 0;
	...
	dealZhengfu: function dealZhengfu(data) {
		var yAxisData = [],
		    pos = [],
		    neg = [],
		    neu = [],
	...
	dealMids: function dealMids(data) {
		data = data || [];
		var xAxisData = [],
		    pos = [],
		    neg = [],
	...
	dealTimeRange: function dealTimeRange() {
		var time = this.state.timeRange,
		    begin,
		    end = Date.now(),
		    delta = 24 * 3600 * 1000;
		if (time == 'today') {
