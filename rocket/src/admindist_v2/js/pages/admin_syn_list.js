...
function init_or_update_list(token) {
    ...
    $.ajax({ url: admin_api + "/api/v1/syndicate",
        ...
        $("span.info-box-icon").each(function(){
          $(this).text($.trim($(this).find("span").text()).substring(0,1).toUpperCase());
        });
        //  绑定函数
...
function init_modal(token,template_list,template_paginate){
        ...
        $('#myModal').unbind('show.bs.modal').on('show.bs.modal', function (event) {
          ...
          if(button.hasClass('add') === true){
            ...
            modal.find(".spr_mgr").hide();
            modal.find(".tel").hide();
            modal.find(".syn_uuid").hide();
            modal.find("input#syn_name").val(button.parent().siblings("span.info-box-number").text());
            modal.find("textarea#syn_info").val(button.parent().siblings("span.progress-description").text());
          }
        $("button#add").unbind('click').bind('click',function(){
            var data = {
                "name":$("#myModal input#syn_name").val(),
                "desc":$("#myModal textarea#syn_info").val(),
                "user_name":$("#myModal input#sup_mgr_name").val(),
                "telephone":$("#myModal input#tel").val(),
                "uuid":$("#myModal input#syn_uuid").val()
            };
            add_syn(token,data,template_list,template_paginate);
            $('#myModal').modal('hide');
        });
        $("button#edit").unbind('click').bind('click', function(){
            var roles = new Array();
            var syn_uuid = button.parent().siblings("span.info-box-text").attr("syn_uuid");
            ...
            update_syn(token,syn_uuid,data,template_list,template_paginate);
            $('#myModal').modal('hide');
        });
    });
}
...
function reset_form(modal){
    modal.find(".spr_mgr").show();
    modal.find(".tel").show();
    modal.find(".syn_uuid").show();
...
//添加syn
function add_syn(token,data,template_list,template_paginate){
    $.ajax({
        url: admin_api + "/api/v1/syndicate",
        data: data,
        type: "POST",
        beforeSend: function beforeSend(xhr) {
            xhr.setRequestHeader('user_token', token);
...
//更新syn
function update_syn(token,syn_uuid,data,template_list,template_paginate){
    $.ajax({
        url: admin_api + "/api/v1/syndicate/"+syn_uuid,
        data: data,
        type: "PUT",
        beforeSend: function beforeSend(xhr) {
            xhr.setRequestHeader('user_token', token);
...
//删除admin
//TODO 也应该是工具方法 传入参数 api+user_id
function delete_syn(syn_uuid, token, template_list, template_paginate) {
    $.ajax({
        url: admin_api + "/api/v1/syndicate/" + syn_uuid,
        data: { "test": "test" },
        type: "DELETE",
