...
function init_or_update_list(token) {
    ...
    $.ajax({ url: admin_api + "/api/v1/super",
        ...
        $(".delete").bind('click', function () {
            if(confirm("确定删除么?")){
                ...
                delete_spr_mgr(user_id, token, template_list, template_paginate);
            }
        });
        //模态框绑定函数
...
function init_modal(token,template_list,template_paginate){
        ...
        $('#myModal').unbind('show.bs.modal').on('show.bs.modal', function (event) {
          ...
          if(button.hasClass('add') === true){
            ...
            modal.find("select option").each(function(){
                if($(this).val() === button.parent().siblings("td.syn").attr("syn_uuid")){
        ...
        $("button#add").unbind('click').bind('click',function(){
            var data = {
                ...
                "syndicate":$("#myModal select option:selected").val()
            };
            add_spr_mgr(token,data,template_list,template_paginate);
            $('#myModal').modal('hide');
        });
        $("button#edit").unbind('click').bind('click', function(){
            ...
            update_spr_mgr(token,data,template_list,template_paginate);
            $('#myModal').modal('hide');
        });
    });
}
...
//添加spr_mgr
function add_spr_mgr(token,data,template_list,template_paginate){
    $.ajax({
        url: admin_api + "/api/v1/super",
        data: data,
        type: "POST",
        beforeSend: function beforeSend(xhr) {
            xhr.setRequestHeader('user_token', token);
...
//更新spr_mgr
function update_spr_mgr(token,data,template_list,template_paginate){
    $.ajax({
        url: admin_api + "/api/v1/super",
        data: data,
        type: "PUT",
...
function delete_spr_mgr(user_id, token, template_list, template_paginate) {
    $.ajax({
        url: admin_api + "/api/v1/super/" + user_id,
        data: { "test": "test" },
        type: "DELETE",
        beforeSend: function beforeSend(xhr) {
            xhr.setRequestHeader('user_token', token);
