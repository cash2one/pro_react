...
define(['mods', paths.rcn.util + '/rest.js', paths.ex.page + '/advices/analy/spread/bar.js', paths.ex.page + '/advices/analy/spread/scatter.js', paths.ex.page + '/advices/analy/spread/pie.js', paths.ex.page + '/advices/analy/spread/force.js', paths.ex.page + '/advices/analy/spread/bar-distributed.js', paths.ex.page + '/advices/analy/spread/fakeData.js'], function (mods, r, Bar, Scatter, Pie, Force, BarDis, fake) {
	...
	var Detail = React.createClass({
		...
		componentDidMount: function componentDidMount() {
			...
			if (this.state.type == 'event') {
				...
				user.read().done(function (user) {
					_this.setState({ loadTotal: 3 });
					_this.getRoute(user);
					// this.getDetail(user);
				});
			}
		...
		getDetail: function getDetail(user) {
			...
			if (type == 'event') {
				...
				rest.event.stat.read(evid).done(function (_ref2) {
					var data = _ref2.data;
					return handler(data);
				});
			} else {
				rest.company.stat.read(user.company_uuid).done(function (_ref3) {
					var data = _ref3.data;
					return handler(data);
				});
			}
				...
				React.createElement(
					...
					React.createElement(
						...
						React.createElement(
							'div',
							{ className: 'mb30' },
							React.createElement(Force, { title: '传播路径', nodes: this.state.rout.nodes, edges: this.state.rout.edges, returnLegend: function returnLegend(legend) {
									return _this7.setState({ legend: legend });
								} })
						),
						React.createElement(
							'div',
