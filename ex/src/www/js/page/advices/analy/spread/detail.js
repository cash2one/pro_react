define([
...
], function(mods, r, Bar, Scatter, Pie, Force, BarDis, fake){
	...
	var Detail = React.createClass({
		...
		getRoute: function(user){
			var nodes = {}, edges = [], lv = 0, lvlimit = 6, type = this.props.route.tp;
			if(type == 'company')
				lvlimit = 2;
			else if(type == 'event')
				lvlimit = 100;
		...
		getDetail: function(user){
			...
			if(type == 'event'){
				let evid = this.state.eventId;
				rest.event.stat.read(evid).done(({data}) => handler(data))
			} else {
				rest.company.stat.read(user.company_uuid).done(({data}) => handler(data))
			}
		},
				...
				<div>
					<div className={"advices-analy-spread-detail" + (this.state.loadEnd && !this.state.isBlank ? '' : ' loading')}>
						...
						<div className="bd">
							...
							<div className="mb30">
								<Force title={'传播路径'} nodes={this.state.rout.nodes} edges={this.state.rout.edges} returnLegend={legend => this.setState({legend})} />
							</div>
							<div className="mb30">
								{this.renderZhuanfa()}
							</div>
